...
function mapLocalPredictions(response) {
  return {
    image: response.image,
    predictions: response.map((p) => ({
      x: p.x,
      y: p.y,
      width: p.width,
      height: p.height,
      confidence: p.score,
      class: classMap.get(p.classId),
      id: uuid(),
    })),
  };
}
...
async function runAutoDetect() {
  try {
    const imgEl = document.querySelector(`img[data-id="${currentImgId}"]`);
    ...
    if (useApi) {
      ...
      currentPredictions = mapLocalPredictions(data);
    }
...
function drawBoundingBox(
  ...
  if (useApi) {
    ...
    scaleX = imageElement.clientWidth / apiWidth;
    scaleY = imageElement.clientHeight / apiHeight;
  }

  // 2. Adjust for Center vs Top-Left origin (Toggle isCenter based on your AI model)
  const isCenter = useApi;
...
async function persistPredictions() {
  ...
  for (let pred of currentPredictions.predictions) {
    ...
    const coordinatesCentered = useApi;
    ...
    const newComp = {
      ...
      value: "",
      desc: "",
      x: coordinatesCentered ? pred.x : pred.x + pred.width/2,
      y: coordinatesCentered ? pred.y : pred.y + pred.height/2,
      imgId: currentImgId,
    };
